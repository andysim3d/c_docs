...
struct _Bit_reference {
    unsigned int *_M_p;   // pointer to data
    unsigned int _M_mask; // mask for what?
    _Bit_reference(unsigned int *__x, unsigned int __y)
        : _M_p(__x), _M_mask(__y) {}

public:
    _Bit_reference() : _M_p(0), _M_mask(0) {}
    operator bool() const { return !(!(*_M_p & _M_mask)); } //为何要取两次反？
    _Bit_reference &operator=(bool __x) {
        if (__x)
            *_M_p |= _M_mask; // 为何要用 mask 和 实际data来计算值？？
        else
            *_M_p &= ~_M_mask;
        return *this;
    }
...
template < class _Allocator, bool __is_static > class _Bvector_alloc_base {
public:
...
template < class _Allocator > class _Bvector_alloc_base< _Allocator, true > {
public:
    typedef typename _Alloc_traits< bool, _Allocator >::allocator_type
        allocator_type;
